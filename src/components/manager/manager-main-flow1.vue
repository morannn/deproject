      ...
      <div :class="`board-left-${value}`" v-for="(value,id) in boxmaxList">
        ...
        <div ref="elem" class="eme">
          <draggable :class="`board-left-${value}`" v-model="value.taskList" :scroll=true group="taskList" animation="100" @start="onStart"
            ...
            <ManagerMainCard @refresh="refresh"  v-if="departmentnum>1" :class="`board-left-${value}`" :title="flow" @remove="remove(i, $event)" v-for="flow in contentList[id].data" :key="flow.id" :data-id="flow.id">
            </ManagerMainCard>
            <ManagerMainCard @refresh="refresh" v-if="departmentnum==1" :class="`board-left-${value}`" :title="flow" @remove="remove(i, $event)" v-for="flow in contentList1[id].data" :key="flow.id" :data-id="flow.id">
            </ManagerMainCard>
          </draggable>
          <!-- 卡片组件 -->
        </div>
        <div>
          <ManagerAddTask @refresh="refresh" @fetchContentList1="fetchContentList1" @respones="add_task"></ManagerAddTask>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    onStart(event) {
      ...
      console.log(this.startColumnName)
    },
    onUpdate(event) {
      // 记录拖拽后的卡片信息
      this.endCardInfo = {
      ...
      console.log(this.endColumnName)
      // 发送请求到后端更新数据库
